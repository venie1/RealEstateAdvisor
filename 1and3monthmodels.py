...
def calculate_yoy(df):
    if df.empty or 'combined_region' not in df.columns or 'median_sale_price' not in df.columns:
        ...
        if 'combined_region' in df.columns:
            df['yoy'] = np.nan
        return df
...
def process_imputation(df, group_col='combined_region', output_filename_prefix='processed'):
    ...
    try:
        ...
        for group in monthly_df[group_col].unique():
            ...
            if len(group_df) > 0:
                split_idx = max(1, int(0.8 * len(group_df)))
...
def prepare_features_3m(train_data, target_col):
...
def train_models_3m(X_train, y_train):
    model_results = {}
...
def run_housing_price_prediction_pipeline(horizon=3):
    ...
    with Timer("Model Training"):
        ...
        preprocessor, X_train, y_train, feature_cols = prepare_features_3m(train_data, target_col)
        model_results = train_models_3m(X_train, y_train)
...
historical_df['property_type'] = historical_df['property_type'].str.lower().map(lambda x: property_mapping_final.get(x, x))
...
df_1m['property_type'] = df_1m['property_type'].str.lower().map(lambda x: property_mapping_final.get(x, x))
...
df_3m['property_type'] = df_3m['property_type'].str.lower().map(lambda x: property_mapping_final.get(x, x))
df_3m['forecast_date'] = pd.to_datetime(df_3m['forecast_date'])

df_combined = pd.concat([df_1m, df_3m], ignore_index=True)

def calculate_yoy_final(row, hist_df):
...
for idx, row in df_combined.iterrows():
    if pd.isna(row['predicted_yoy']):
        df_combined.at[idx, 'predicted_yoy'] = calculate_yoy_final(row, historical_df)
